...
public class FrOrderPending extends BaseFragment implements LoadMoreExpandableListView.OnLoadMoreListener {
    ...
    private void getOrderAPI(final boolean isLoadMore) {
        if (CommonUtils.isInternetOn(getContext())) {
            ...
            Observable<OrderData> signupusers = FetchServiceBase.getFetcherService(context)
                    .getOrders(preferences.getUserDataPref().getId(), CommonUtils.converRequestBodyFromMap(params));
